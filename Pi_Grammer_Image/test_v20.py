...
def clean_results():
        ...
        if (os.path.exists('/home/pi/SERIAL_UPLOAD/pi_serial_upload.sh') == True):
                ...
                f.truncate()
                f.close()
        if (os.path.exists('/home/pi/eeprom/pi_eeprom_program.sh') == True):
...
############################################################################################
def program_eeprom():
        command = "sh /home/pi/eeprom/pi_eeprom_program.sh"
        import subprocess
        process = subprocess.Popen(command.split(), stdout=subprocess.PIPE)
        output = process.communicate()[0]
        print output
...
#############################################################################
def parse_results_eeprom():
        #variables to store success/failure of serial flash writing
        # NOTE, I am NOT verifying flash on serial upload to decrease programming time by 50%
        # So we are going to show a successful serail program, but only seeing that it was written
        eeprom_flash_written = False

        f = open('/home/pi/eeprom/eeprom_results.txt', 'r')
        FLASH_SIZE = '00000'    # the flash size can vary
...
while True:
        ...
        if GPIO.input(CAPSENSE) == True:
                ...
                program()
                parse_results()
                if ((os.path.exists('/home/pi/eeprom/pi_eeprom_program.sh') == True) and (LOCK_BITS_PASS == True)):
                        program_eeprom()
                        parse_results_eeprom()
                        #time.sleep(3)
